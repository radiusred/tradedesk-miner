//! Phase 4 Plan 04-06 — ANOM-08 `stats.heteroskedasticity.arch_lm@1` happy-
//! path integration test.
//!
//! Pattern analog: `crates/miner-core/tests/scan_adf.rs` (sibling ANOM-family
//! integration test). Pins envelope shape via `insta` snapshot. Hand-derived
//! ARCH-LM test statistic tolerance: 1e-10; chi²(L) p approx 1e-8 per
//! RESEARCH §Section 2 — full statsmodels golden parity lands in Plan 04-11.
...
use miner_core::scan::anom::ArchLmScan;
...
/// Plan 04-06 Task 1 — happy-path envelope snapshot on a deterministic LCG
/// fixture. Plan 04-11 will pin the actual ARCH-LM stat/p-value against
/// `statsmodels.stats.diagnostic.het_arch` within 1e-10 (stat) / 1e-8 (p).
#[test]
fn scan_arch_lm_happy_path() {
    let closes = lcg_closes(80, 42);
    let bars = build_bar_frame_from_closes(&closes);

    let resolved_params = serde_json::json!({"lag": 5});
    ...
    let req = ScanRequest {
        scan_id: "stats.heteroskedasticity.arch_lm".into(),
        version: 1,
    ...
    ArchLmScan.run(&ctx, &req, &mut sink).expect("scan ok");
    ...
    assert!(r.effect.p_value.is_some(), "ARCH-LM emits p_value");
    ...
    insta::assert_json_snapshot!("arch_lm_happy_path", masked);
}
