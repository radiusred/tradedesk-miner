//! Phase 4 Plan 04-04 — ANOM-11 `stats.drawdown.profile@1` happy-path
//! integration test.
//!
...
//! Hand-derived V-shape tolerance: 1e-12 (pure arithmetic kernel).
...
use miner_core::scan::anom::DrawdownProfileScan;
...
/// Plan 04-04 Task 3 — happy-path envelope snapshot on a deterministic LCG.
#[test]
fn scan_drawdown_happy_path() {
    ...
    let req = ScanRequest {
        scan_id: "stats.drawdown.profile".into(),
        version: 1,
    ...
    // dd_distribution_p50_p95_p99 always has length 3.
    assert_eq!(r.effect.extra["dd_distribution_p50_p95_p99"].shape, vec![3]);

    let mut masked = serde_json::to_value(&findings[0]).expect("envelope -> Value");
    common::mask_volatile_fields(&mut masked);
    insta::assert_json_snapshot!("drawdown_happy_path", masked);
}
