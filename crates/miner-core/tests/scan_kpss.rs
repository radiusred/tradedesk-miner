//! Phase 4 Plan 04-05 — ANOM-06 `stats.stationarity.kpss@1` happy-path
//! integration test.
//!
//! Pattern analog: `crates/miner-core/tests/scan_adf.rs` (sibling
//! stationarity-test integration test). Pins envelope shape via `insta`
//! snapshot. Reference: `statsmodels.tsa.stattools.kpss(x, regression='c',
//! nlags='auto')`. Tolerance pinned for goldens in Plan 04-11 (1e-8 on
//! interpolated p; 1e-10 on stat).
...
use miner_core::scan::anom::KpssScan;
...
/// Plan 04-05 Task 2 — happy-path envelope snapshot.
#[test]
fn scan_kpss_happy_path() {
    let closes = random_walk_closes(80, 42);
    let bars = build_bar_frame_from_closes(&closes);

    let resolved_params = serde_json::json!({"regression": "c", "nlags": "auto"});
    ...
    let req = ScanRequest {
        scan_id: "stats.stationarity.kpss".into(),
        version: 1,
    ...
    KpssScan.run(&ctx, &req, &mut sink).expect("scan ok");
    ...
    assert!(r.effect.p_value.is_some(), "KPSS emits p_value");
    // KPSS stat is always non-negative.
    ...
    insta::assert_json_snapshot!("kpss_happy_path", masked);
}
