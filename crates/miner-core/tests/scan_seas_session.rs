...
use miner_core::scan::seas::SessionScan;
use miner_core::scan::{Scan, ScanCtx, ScanRequest};

use common::BufferSink;

#[test]
fn scan_seas_session_happy_path() {
    // 7 days × 24h × 4 bars/h at 15m = 672 bars.
    let bars = build_synthetic_15m_bars(672, 0xCAFE_BABE);
    ...
    let req = ScanRequest {
        scan_id: "seas.bucket.session".into(),
        version: 1,
    ...
    // 5 numeric arrays length 4 (default FX-major sessions).
    ...
    // bucket_labels carries the 4 FX-major session names.
    let labels_bytes = &r.effect.extra["bucket_labels"].data.0;
    let labels: Vec<String> =
        serde_json::from_slice(labels_bytes).expect("bucket_labels JSON parse");
    assert_eq!(labels, vec!["asia", "london", "ny", "overlap"]);
    // session_boundaries_utc carries the four `{name, start, end}` objects.
    let boundaries_bytes = &r.effect.extra["session_boundaries_utc"].data.0;
    let boundaries: Vec<serde_json::Value> = serde_json::from_slice(boundaries_bytes).unwrap();
    ...
    insta::assert_json_snapshot!("scan_seas_session_happy_path", masked);
}
