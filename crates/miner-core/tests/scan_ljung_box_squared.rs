//! Phase 4 Plan 04-04 — ANOM-04 squared variant
//! (`stats.autocorr.ljung_box_sq@1`) happy-path integration test.
//!
...
use miner_core::scan::anom::LjungBoxSqScan;
...
/// Plan 04-04 Task 1 — happy-path envelope snapshot.
#[test]
fn scan_ljung_box_squared_happy_path() {
    let closes = lcg_closes(64, 42);
    let bars = build_bar_frame_from_closes(&closes);

    let resolved_params = serde_json::json!({"lags": 5});
    ...
    let req = ScanRequest {
        scan_id: "stats.autocorr.ljung_box_sq".into(),
        version: 1,
    ...
    // 64 closes -> 63 returns -> 63 squared returns.
    assert_eq!(r.effect.n, Some(63));

    // series_kind discriminator round-trips through the wire form.
    let sk_bytes = &r.effect.extra["series_kind"].data.0;
    let series_kind = std::str::from_utf8(sk_bytes).expect("utf-8");
    assert_eq!(series_kind, "squared_returns");
    ...
    insta::assert_json_snapshot!("ljung_box_squared_happy_path", masked);
}
