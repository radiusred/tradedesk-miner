...
- [ ] **CACHE-05**: User can trust that miner surfaces Dukascopy's `volume` field as `tick_volume: f64` — the per-bar SUM of per-tick `bid_vol`/`ask_vol` floats that `tradedesk-dukascopy` produces via `vol.resample(rule).sum()`, not contract/notional volume — and the 00-indexed month layout is documented + boundary-tested
