...
- **5-minute and 10-minute aggregation timeframes.** `Timeframe` gains `Tf5m` / `Tf10m` variants (wire forms `"5m"` / `"10m"`); the aggregator, misalignment validator, bar-cache path layout, Arrow IPC metadata, sweep manifest, and the `miner scan --timeframe` CLI all now accept the five timeframes `5m | 10m | 15m | 1h | 1d`. The `Timeframe` JSON schema surfaced by `miner scans` enumerates all five. Both 5m and 10m divide the hour and day evenly, so bucketing stays partial-bucket-free and DST-invariant (UTC-only). (RAD-3630.)
